...
var sessionCookieSecret = flag.String("session_cookie_secret", "", "A string used to encrypt session cookies")
...
func main() {
	...
	sm := session.NewManager(session.NewMemoryStore(), *sessionCookieSecret, func() string { return uuid.New().String() }, *sessionDuration)
