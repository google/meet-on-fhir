...
// GetStringOrEmpty returns the string value for the given key.
// Returns an empty string if the value type is not string.
func (s *Session) GetStringOrEmpty(key string) string {
	if s.Value == nil {
		return ""
	}
	v, ok := s.Value[key].(string)
	if !ok {
		return ""
	}
	return v
